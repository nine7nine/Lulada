...
/* Python.h include dropped — vht used it for the PyObject return type
 * of track_get_rec_update; that function and its caller chain are gone. */
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
...
/* track_get_rec_update removed: returned a Python dict in upstream vht
 * for the GTK UI's recording indicator. Element-NSPA tracker reads
 * `trk->updates` / `cur_rec_update` directly from C++ side. */
...
/* track_get_envelope removed: returned a Python list of envelope nodes
 * for the GTK editor. Element-NSPA reads `trk->env[c]->nodes[]` directly. */
