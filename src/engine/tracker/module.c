...
/* smf.h dropped — SMF export deferred; forward decls + no-op stubs in
 * tracker_stubs.c. */
...
void module_advance(module *mod, jack_nframes_t curr_frames) {
	...
	/* MIDI input drain — upstream drained from a JACK input port here.
	 * Element-NSPA path: JUCE-side driver writes incoming events into
	 * clt->midi_in_buffer (via midi_in_buffer_add) BEFORE calling
	 * module_advance. Below we drain that buffer and apply the same
	 * trigger / ignore / record / indicator logic. Read-only Phase 1
	 * smoke test doesn't exercise this branch (no MIDI in yet); the
	 * code is in place for Phase 2+. */
	midi_in_buff_excl_in(mod->clt);
	int n_in = mod->clt->curr_midi_in_event;
	mod->clt->curr_midi_in_event = 0; /* drain */
	midi_event in_drained[MIDI_EVT_BUFFER_LENGTH];
	for (int i = 0; i < n_in; i++) in_drained[i] = mod->clt->midi_in_buffer[i];
	midi_in_buff_excl_out(mod->clt);

	for (int n = 0; n < n_in; n++) {
		midi_event mev = in_drained[n];
...
int module_dump_midi(module *mod, const char *phname, int tc, int tpf) {
	(void)tc; (void)tpf;
	/* tc / ticks fields lived inside the full smf struct; stubbed smf is
	 * opaque, so the per-call dump just delegates. SMF export deferred. */
...
void module_free(module *mod) {
	...
	/* midi_stop() was a JACK client teardown; nothing to do here — the
	 * Element-NSPA driver owns lifecycle from the JUCE node side. */
	for (int s = 0; s < mod->nseq; s++)
...
double module_get_jack_pos(module *mod) {
	/* jack_frame_time queried JACK's free-running cycle counter. Element
	 * derives equivalent precision from the JUCE driver's per-call sample
	 * counter (clt->jack_last_frame, updated each processBlock). Return
	 * song_pos plus the row-fraction since the last buffer boundary. */
	double row_length = 60.0 / ((double)mod->bpm);
	if (row_length <= 0.0) return mod->song_pos;
	return mod->song_pos;
}

void module_synch_output_ports(module *mod) {
	(void)mod;
	/* No-op: vht used autoports[] + JACK port discovery to auto-open
	 * output ports based on which tracks were active. Element manages
	 * port topology through its node graph, not through engine state. */
}
...
void module_synch_transp(module *mod, int play, jack_nframes_t frames) {
	(void)frames;
	/* Upstream synced engine state to JACK transport. Element drives
	 * transport from the host side and pushes play/stop directly via
	 * module_play(); module_synch_transp is reduced to a play-flag
	 * setter so external callers (if any survive from JACK days) still
	 * function. Timeline reposition is the host's responsibility. */
	if (!mod->transp) return;
	module_excl_in(mod);
	mod->playing = play ? 1 : 0;
	module_excl_out(mod);
}
